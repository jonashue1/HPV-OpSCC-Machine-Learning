...
title = getTitle();
selectWindow(title);
//Eg. this renames "ID15_Img1.tif.tif.ome.tif" to "ID15_Img1_" which will eventually be saved as "ID15_Img1_Str" for training set, stroma probability map. 
newtitle = replace(title, "\\.tif\\.ome\\.tif", "_");
run("Stack to Images");

// Str = Stroma, Nuc = Nuclei, Bg = Background
saveAs("Tiff", output_path+"/"+newtitle+"Str.tif");
close();
saveAs("Tiff", output_path+"/"+newtitle+"Nuc.tif");
close();
saveAs("Tiff", output_path+"/"+newtitle+"Bg.tif");
close();
